...
function ProductRow({ product, onSaved }) {
  ...
  const save = useAction(() => api.updateProduct(product.id, {
    basePrice: Number(price), stockQty: Number(qty), supplierSku: sku.trim() || undefined,
  }));
  const dirty = Number(price) !== Number(product.base_price)
    || Number(qty) !== Number(product.stock_qty)
...
function LedgerView() {
  const { data, loading, error, reload } = useFetch(() => api.myLedger(), []);
      ...
      {loading ? <Spinner /> : error ? <ErrorState message={error} onRetry={reload} /> : (
        <>
          <div className="balance-hero">
            ...
            <b className={Number(data.balance) < 0 ? "balance-negative" : "balance-positive"}>
              {money(Math.abs(data.balance))} {Number(data.balance) < 0 ? "(مستحق لك)" : "(مستحق عليك)"}
            </b>
          </div>
          ...
          <div className="ledger-list">
            {!data.entries?.length ? (
              ...
              <div className="order-row" key={e.id} style={{ cursor: e.receipt_id ? "pointer" : "default" }}
                onClick={() => e.receipt_id && window.open(api.receiptPdfUrl(e.receipt_id), "_blank")}>
                <div className="order-row-top">
                  <span className="order-row-id">{e.description}</span>
                  <span className={"status-pill" + (e.direction === "credit" ? "" : " status-pill-debit")}>
                    {e.direction === "credit" ? "+ " : "− "}{money(e.amount)}
                  </span>
                </div>
                <span className="order-row-meta">
                  {String(e.entry_date).slice(0, 10)}
                  {e.receipt_id && " · اضغط لتحميل الواصل PDF"}
                </span>
              </div>
            ))}
          </div>
...
function ImportProductsView({ sections, onClose, onImported }) {
        ...
        try {
          ...
          const cleaned = json
            ...
            .map((r) => ({
              ...
              supplierSku: String(r["رقم الصنف عندك (اختياري)"] || "").trim() || undefined,
            }));
          resolve(cleaned);
  ...
  const submit = useAction(() => api.importProducts(
    rows.map((r) => ({ ...r, sectionId: sections.find((s) => s.name === r.sectionName)?.id }))
  ));

  const unmatched = rows?.filter((r) => !sections.find((s) => s.name === r.sectionName)) ?? [];
        ...
        <>
          ...
          {unmatched.length > 0 && (
            <div className="note-block" style={{ marginTop: 8 }}>
              ...
              <p>{unmatched.length} صنف باسم قسم غير مطابق لأقسامك المعتمدة وسيتم تجاهلهم: {unmatched.map((r) => r.sectionName).join("، ")}</p>
            </div>
          )}
          {submit.error && <p className="field-error">{submit.error}</p>}
          <button className="btn-primary" disabled={submit.pending}
            onClick={() => submit.run().then(onImported).catch(() => {})}>
            {submit.pending ? "جارٍ الاستيراد…" : `استيراد ${rows.length - unmatched.length} صنف`}
          </button>
        </>
      )}
      <button className="btn-ghost" onClick={onClose}>إلغاء</button>
